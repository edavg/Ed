...
}, "D:/a/ed-portfolio/src/components/BentoGrid.astro", void 0);
...
const $$BentoGridItem = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<a${addAttribute(`/proyectos/${id}`, "href")}${addAttribute(`row-span-1 rounded-xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none p-4 bg-slate-600/20 hover:bg-slate-500/20 dark:border-white/[0.2] border border-transparent justify-between flex flex-col space-y-4 cursor-pointer ${className}`, "class")}> <div${addAttribute(`view-transition-name: image-${id}`, "style")} class="w-full max-h-[130px]"> ${renderComponent($$result, "Image", $$Image, { "src": image, "width": 250, "height": 200, "alt": "", "class": "object-cover w-full h-full object-top" })} </div> <div${addAttribute(`view-transition-name: svg-${id}`, "style")} class="group-hover/bento:translate-x-2 transition duration-200"> ${renderComponent($$result, "Image", $$Image, { "src": icon, "width": 30, "height": 30, "alt": "" })} <div${addAttribute(`view-transition-name: title-${id}`, "style")} class="font-sans font-bold text-neutral-600 dark:text-neutral-200 mb-2 mt-2"> ${title} </div> <div${addAttribute(`view-transition-name: description-${id}`, "style")} class="font-sans font-normal text-neutral-600 text-xs dark:text-neutral-300 line-clamp-1"> ${description} </div> </div> </a>`;
}, "D:/a/ed-portfolio/src/components/BentoGridItem.astro", void 0);
...
}, "D:/a/ed-portfolio/src/components/BentoGridDemo.astro", void 0);
